...
namespace Game
{
    public class GameManager : MonoBehaviour
    {
        ...
        {
            RequestGenes(_idInput.text);
        }

        void RequestGenes(string inputText)
        {
            if (string.IsNullOrEmpty(inputText) || _isFetchingGenes) return; //prevent duplicate request
            _isFetchingGenes = true;
            StartCoroutine(GetAxiesGenes(inputText));
        }
